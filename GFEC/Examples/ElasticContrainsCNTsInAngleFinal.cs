...
namespace GFEC
{
    public static class ElasticContrainsCNTsInAngleFinal
    {
        private const int totalNodes = 648;
        private const int AddedNodes = 1;
        private const int RodElements = AddedNodes;
        private const int totalContactElements = 10;/*78;*//*61;*//*28;*///10;//42;
        ...
        private static double offset = 0.1 + (nodesInXCoor - 1) * xIntervals * Math.Sin(angle) - (totalContactElements - 1) * xIntervals * Math.Sin(angle); //6.95;//8.1;//9.3// tested: 7.0 - 0.05, 7.0 -0.45, 7.0 -0.25, 7.0 -0.0
        //private static double offset = (nodesInXCoor - 1) * xIntervals * Math.Sin(angle) - (totalContactElements - 1) * xIntervals * Math.Sin(angle); //6.95;//8.1;//9.3// tested: 7.0 - 0.05, 7.0 -0.45, 7.0 -0.25, 7.0 -0.0
        private static double offsetInY = (nodesInXCoor - 1) * xIntervals * Math.Cos(angle);
        private static double gap = offsetInY; //tested: 1.14, 2.75, 2.10, 0.75
        ...
        //External loads
        const double externalStructuralLoad = -2.85;//2.85 -->20 Mpa
        const double T0 = 100.0;
        const double cond = 3300;
        //static double cosTheta = Math.Cos(angle);
        //static double SinTheta = - Math.Sin(angle);
        ...
        //static double externalHeatLoad = 2 * T0 * (cond / (6 * xIntervals * yIntervals)) * ((Math.Pow(cosTheta,2) - Math.Pow(SinTheta, 2) + 2 * SinTheta * cosTheta) * (Math.Pow(xIntervals, 2) - 2 * Math.Pow(yIntervals, 2)) + 2 * (Math.Pow(xIntervals, 2) + Math.Pow(yIntervals, 2)));
        ...
        private static void CreateStructuralBoundaryConditions()
        {
            ...
            for (int i = totalNodes / 2 + 1; i <= totalNodes / 2 + nodesInXCoor; i++)
            {
                boundedDofs.Add(i * 2 + 0);
                boundedDofs.Add(i * 2 - 1); //lower beam support for all lower boundary nodes
            }
            for (int i = nodesInXCoor * nodesInYCoor + 2 * nodesInXCoor; i <= totalNodes; i+= nodesInXCoor)
            {
                //boundedDofs.Add(i * 2 + 0);
                boundedDofs.Add(i * 2 - 1); //lower beam support for all right boundary nodes
            }
            for (int i = nodesInXCoor * nodesInYCoor + nodesInXCoor + 1; i <= totalNodes - nodesInXCoor + 1; i += nodesInXCoor)
            {
                //boundedDofs.Add(i * 2 + 0);
                boundedDofs.Add(i * 2 - 1); //lower beam support for all left boundary nodes
            }
        ...
        private static Dictionary<int, INode> CreateNodes()
        {
            ...
            double anSin = Math.Sin(angle);
            nodes[totalNodes + 1] = new Node(ancos * yIntervals * (nodesInYCoor - 1), anSin * (nodesInYCoor - 1) * yIntervals + yIntervals);
        ...
        private static Dictionary<int, Dictionary<int, int>> CreateConnectivity()
        {
            ...
            //Rod elements
            count = connectivity.Count;
            connectivity[count + 1] = new Dictionary<int, int>() { { 1, 244 }, { 2, totalNodes + AddedNodes } };
        ...
        private static Dictionary<int, IElementProperties> CreateElementProperties()
        {
            double E = YoungMod;
            double A = area;
            double contactArea = thickness * xIntervals;
            ...
            {
                elementProperties[i] = new ElementProperties(E, contactArea, type2);
                elementProperties[i].Density = density;
                elementProperties[i].Thickness = thickness;
            }
            ...
            {
                elementProperties[i] = new ElementProperties(rodElementsYoungMod, rodElementsSectionArea, type3);
            }
        ...
        private static Dictionary<int, IElementProperties> CreateThermalElementProperties()
        {
            ...
            for (int i = totalElements + 1; i <= totalElements + totalContactElements - 1 + totalContactElements - 2 + totalContactElements - 3 + totalContactElements - 4; i++)
            {
                ...
                elementProperties[i].ThermalConductivity = thermalCond;
                elementProperties[i].SectionArea = contactArea;
        ...
        public static Results RunStaticExample()
        {
            ...
            {
                if(dof == nodesInXCoor * nodesInYCoor * 2)
                {
            ...
            for (int i = 1; i <= allStepsSolutions.Count; i++)
            {
                ...
                for (int j = totalElements + 1; j <= totalElements + 4 * totalContactElements - 10; j++)
                {
