...
namespace GFEC
{
    public static class TwoBlocks2DNtN
    {
        ...
        const double YoungMod2 = 1.0 * 1e5;
        ...
        private static Dictionary<int, INode> CreateNodes()
        {
            ...
            k += 1;
            nodes[k] = new Node(3*length, 0.0);
            k += 1;
            nodes[k] = new Node(0.0, length);
            k += 1;
            nodes[k] = new Node(length, length);
            k += 1;
            nodes[k] = new Node(2 * length, length);
            k += 1;
            nodes[k] = new Node(3 * length, length);
            k += 1;
            //Second block
            nodes[k] = new Node(length, -length - gap);
            k += 1;
            nodes[k] = new Node(2 * length, -length - gap);
            k += 1;
            nodes[k] = new Node(length, -gap);
            k += 1;
            nodes[k] = new Node(2*length, -gap);
            k += 1;
            //Bars
        ...
        private static Dictionary<int, Dictionary<int, int>> CreateConnectivity()
        {
            ...
            int k = 1;
            connectivity[k] = new Dictionary<int, int>() { { 1, 1 }, { 2, 2 }, { 3, 6 }, { 4, 5 } };
            k += 1;
            connectivity[k] = new Dictionary<int, int>() { { 1, 2 }, { 2, 3 }, { 3, 7 }, { 4, 6 } };
            k += 1;
            connectivity[k] = new Dictionary<int, int>() { { 1, 3 }, { 2, 4 }, { 3, 8 }, { 4, 7 } };
            k += 1;
            connectivity[k] = new Dictionary<int, int>() { { 1, 9 }, { 2, 10 }, { 3, 12 }, { 4, 11 } };
            k += 1;
            //bar elements
            connectivity[k] = new Dictionary<int, int>() { { 1, 5 }, { 2, 13 } };
            k += 1;
            connectivity[k] = new Dictionary<int, int>() { { 1, 8 }, { 2, 14 } };
            k += 1;
            //Contact elements
            connectivity[k] = new Dictionary<int, int>() { { 1, 11 }, { 2, 2 } };
            k += 1;
            connectivity[k] = new Dictionary<int, int>() { { 1, 12 }, { 2, 3 } };
            return connectivity;
        }
        ...
        private static Dictionary<int, IElementProperties> CreateElementProperties()
        {
            ...
            for (int i = 5; i <= 6; i++)
            {
                elementProperties[i] = new ElementProperties(E2, A, type3);
        ...
        public static Results RunStaticExample()
        {
            ...
            for (int i = 1; i <= allStepsSolutions.Count; i++)
            {
                ...
                Vector[2] = allStepsContactForces.Single(m => m.Key == i).Value.Single(n => n.Key == 7).Value[2];
                Vector[3] = allStepsContactForces.Single(m => m.Key == i).Value.Single(n => n.Key == 7).Value[3];
                Vector[4] = allStepsContactForces.Single(m => m.Key == i).Value.Single(n => n.Key == 8).Value[0];
                Vector[5] = allStepsContactForces.Single(m => m.Key == i).Value.Single(n => n.Key == 8).Value[1];
                Vector[6] = allStepsContactForces.Single(m => m.Key == i).Value.Single(n => n.Key == 8).Value[2];
                Vector[7] = allStepsContactForces.Single(m => m.Key == i).Value.Single(n => n.Key == 8).Value[3];
